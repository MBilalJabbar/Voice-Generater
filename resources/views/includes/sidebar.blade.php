 <!-- SIDEBAR -->

 <aside class="app-sidebar sticky" id="sidebar">

     <!-- Start::main-sidebar-header -->
     <div class="main-sidebar-header">
         <a href="" class="header-logo">
             <img class="mb-2 mt-3" src="{{ asset('assets/images/Group 1000007299@3x.png') }}" alt="Task Image"
                 style="width: 120px; height: 80px; object-fit: contain;">


             {{-- <img src="/.svg" alt="logo" class="toggle-dark">
            <img src="/assets/images/others/logo.svg" alt="logo" class="desktop-white">
            <img src="/assets/images/others/logo.svg" alt="logo" class="toggle-white"> --}}
         </a>
     </div>

     <!-- Start::main-sidebar -->
     <div class="main-sidebar" id="sidebar-scroll">
         <!-- Start::nav -->
         <nav class="main-menu-container nav nav-pills flex-column sub-open">
             <div class="slide-left" id="slide-left">
                 <svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191" width="24" height="24"
                     viewBox="0 0 24 24">
                     <path d="M13.293 6.293 7.586 12l5.707 5.707 1.414-1.414L10.414 12l4.293-4.293z"></path>
                 </svg>
             </div>
             <li class="slide__category"><span class="category-name">Main</span></li>
             <ul class="main-menu d-flex flex-column h-100">
                 <!-- Top Menu Items -->
                 <li class="slide">
                     <a href="{{ url('index') }}" class="side-menu__item">
                         <i class="fas fa-tachometer-alt side-menu__icon"></i>
                         <span class="side-menu__label">Dashboard</span>
                     </a>
                 </li>

                 <li class="slide mt-2">
                     <a href="{{ url('voices-Page') }}" class="side-menu__item">
                         <i class="fa-solid fa-volume-high side-menu__icon"></i>
                         <span class="side-menu__label">Voices</span>
                     </a>
                 </li>

                 <li class="slide mt-2">
                     <a href="{{ url('genrate-audio') }}" class="side-menu__item">
                         <i class="fa-solid fa-microphone-lines side-menu__icon"></i>
                         <span class="side-menu__label">Generate Audio</span>
                     </a>
                 </li>

                 <li class="slide mt-2">
                     <a href="{{ url('genrate-bulk-audio') }}" class="side-menu__item">
                         <i class="fas fa-layer-group side-menu__icon"></i>
                         <span class="side-menu__label">Bulk Generate Audio</span>
                     </a>
                 </li>

                 <li class="slide mt-2">
                     <a href="{{ url('clone-voice') }}" class="side-menu__item">
                         <i class="fa-solid fa-clone side-menu__icon"></i>
                         <span class="side-menu__label">Clone Your Voice</span>
                     </a>
                 </li>

                 <li class="slide mt-2">
                     <a href="{{ url('task-history') }}" class="side-menu__item">
                         <i class="fas fa-clock-rotate-left side-menu__icon"></i>
                         <span class="side-menu__label">Task History</span>
                     </a>
                 </li>



                 <!-- Bottom Menu Items -->
                 <li class="slide mt-2" style="margin-top: 190px !important;">
                     <a href="{{ url('contact') }}" class="side-menu__item">
                         <i class="fas fa-headset side-menu__icon"></i>
                         <span class="side-menu__label">Support</span>
                     </a>
                 </li>

                 <!-- Logout -->
                 <li class="slide mt-2">

                     <a href="/LogoutUser" class="side-menu__item"
   onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
   <i class="fas fa-right-from-bracket side-menu__icon"></i>
   <span class="side-menu__label">Logout</span>
</a>

<form id="logout-form" action="{{ url('/LogoutUser') }}" method="POST" style="display: none;">
    @csrf
</form>

                 </li>
             </ul>


             <div class="slide-right" id="slide-right"><svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191"
                     width="24" height="24" viewBox="0 0 24 24">
                     <path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path>
                 </svg>
             </div>
         </nav>
     </div>
 </aside>
